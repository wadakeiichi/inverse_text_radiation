...
![連続を作る三素過程の準位図。横線が束縛準位、上部の網掛け帯が連続状態（$E>0$）。束縛-束縛（線、第VII部）と対比して、束縛-自由（光電離↔再結合）と自由-自由（制動放射）が連続帯を使うことを示す。第11章 §11.2／第21章 §21.2 の二準位＋三過程図の姉妹図。](../images/continuum-processes.svg){#fig-continuum-processes width=88%}
...
自由電子がイオンの Coulomb 場で加速度を受けると、加速電荷は電磁波を放射する（Larmor 公式、第13章 §13.5b）。これが **自由-自由放射**（制動放射）である。

ここで放出の量を表す係数を二つ区別しておく。**放出係数**（emission coefficient）$j_\nu$ は単位体積・単位振動数・**単位立体角あたり**に放出されるエネルギー（次元は erg s⁻¹ cm⁻³ Hz⁻¹ sr⁻¹）であり、放射輸送方程式にソース項として現れる。これに対し、**体積放射率** $\varepsilon_\nu$ は全立体角に放出されるエネルギー（次元は erg s⁻¹ cm⁻³ Hz⁻¹）で、等方放射では両者は立体角積分 $\varepsilon_\nu = 4\pi\, j_\nu$ で結ばれる。
...
$$
\alpha_\nu^{\mathrm{ff}} \;\propto\; n_e\,n_i\,Z^2\,T_e^{-1/2}\,\nu^{-3}\,\bar g_{\mathrm{ff}}\,\bigl(1-e^{-h\nu/k_BT_e}\bigr)
$$ {#eq-ff-absorption-micro}

である。括弧の $(1-e^{-h\nu/k_BT_e})$ は **誘導放出補正**（第11章・第21章 §21.2 と同じ因子）である。低振動数（Rayleigh–Jeans 側、$h\nu\ll k_BT_e$）ではこの括弧が $\approx h\nu/k_BT_e$ と展開でき、それを吸収した実効形が第17章 §17.6 で使った電波領域の $\alpha_\nu^{\mathrm{ff}}\propto T_e^{-3/2}\nu^{-2}$ である ― 指数がひとつずつずれて見えるのは誘導放出補正を「中に畳み込んだか、外に出して書いたか」の違いにすぎない。ソース関数は放出係数（単位立体角あたり）と吸収係数の比 $S_\nu = j_\nu/\alpha_\nu$ で定義されるので、等方放射で $j_\nu^{\mathrm{ff}} = \varepsilon_\nu^{\mathrm{ff}}/4\pi$ を用いる。@eq-ff-emissivity と @eq-ff-absorption-micro の比をとると、$n_e,n_i$ も $\bar g_{\mathrm{ff}}$ も消えて
...
観測スペクトルの形を決めるのは、ここから先の **光学的厚さ** $\tau_\nu=\int\alpha_\nu^{\mathrm{ff}}\,ds$ である。@eq-ff-absorption-micro が低振動数側で $\nu^{-2}$（RJ 極限の実効形）として効くことにより、
...
![自由-自由放射のスペクトル。低振動数では光学的に厚く $F_\nu\propto\nu^2$（電子温度 $T_e$ の黒体の Rayleigh–Jeans 裾）、ターンオーバー $\nu_t$ を越えると光学的に薄くほぼ平坦（$\propto\nu^{-0.1}$）、$h\nu\sim k_BT_e$ で指数カットオフ。電波 HII 領域と銀河団 X 線は同じ熱的制動放射の両端である。](../images/continuum-ff-spectrum.svg){#fig-continuum-ff-spectrum width=92%}
...
::: {.callout-note}
**観測との対応**：太陽スペクトルの連続光のなだらかな形（可視で最大、紫外・赤外へ向けて落ちる）は、H⁻ の吸収係数の波長依存（$\sim0.85\ \mu$m にピーク）でよく説明される。恒星大気モデル（第15章 §15.3、Mihalas 1978）では H⁻ が G・K 型星の連続不透明度の標準的な第一成分として組み込まれている。
:::
...
![同じしきい値（Lyman 912 Å・Balmer 3646 Å・Paschen 8204 Å）が、恒星大気では **束縛-自由の吸収端**（上：短波長へ向け急に立ち上がり $\propto\nu^{-3}$ で減衰、可視は H⁻ が担う）として、星雲では **自由-束縛の放出端＝再結合連続光**（下：各しきいで放出が跳ね上がり短波長へ Wien 減衰、Balmer ジャンプが顕著）として現れる。背景光の有無で吸収にも放出にも見える（§19.6）。](../images/continuum-edges-recomb.svg){#fig-continuum-edges-recomb width=94%}
...
という連鎖で、連続の吸収・放射係数を **自己無撞着** に与える。LTE が成り立つ限り、この連鎖の出力はつねに $S_\nu=B_\nu(T)$（Kirchhoff、@eq-ff-kirchhoff）に帰着する ― 素過程が何であってもソース関数は局所温度の黒体になる、というのが LTE 連続放射の要点である。
...
::: {.callout-note}
**観測との対応**：恒星大気では Balmer 端は **吸収側の段差**（端より青で不透明度増）として、星雲では同じ $3646$ Å に **再結合連続光の放出ジャンプ** として現れる。同じしきい値が、幾何条件（背景光の有無）で吸収にも放出にも見える ― これは線スペクトルで同じ遷移が吸収線にも輝線にもなる対比（[第20章 §20.3](../part7/20-where-lines.qmd#sec-where-lines-emission)）と同じ論理である。
:::
...
[解答例を見る（問題 19-1）](#sol-19-1){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::
...
**関連**：[§19.3 束縛-自由](#sec-continuum-bound-free)、[§19.4 再結合](#sec-continuum-recombination)／吸収線と輝線の幾何は[第20章 §20.3](../part7/20-where-lines.qmd#sec-where-lines-emission)、線のソース関数は[第21章 §21.3](../part7/21-line-formation.qmd#sec-line-source-function)。

::: {.content-visible when-format="html"}
[解答例を見る（問題 19-2）](#sol-19-2){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::
...
::: {.content-visible when-format="html"}
[解答例を見る（問題 19-3）](#sol-19-3){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::
...
::: {.content-visible when-format="html"}
[解答例を見る（問題 19-4）](#sol-19-4){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::
...
::: {.content-visible when-format="html"}
[解答例を見る（問題 19-5）](#sol-19-5){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

## 解答例 {#sec-continuum-micro-solutions .unnumbered}

各問の「解答例を見る」ボタンを押すと、右側のパネルに解答例が表示され、問題文と見比べられる（背景は暗くならず本文はそのまま読める）。印刷版・EPUB版では下に順に掲載される。

::: {.content-visible when-format="html"}
[問題 19-1](#sol-19-1){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 19-2](#sol-19-2){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 19-3](#sol-19-3){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 19-4](#sol-19-4){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 19-5](#sol-19-5){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

::: {#sol-19-1 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-19-1-label"}
::: {.offcanvas-header}
[**解答例（問題 19-1）**]{#sol-19-1-label .offcanvas-title}
...
::: {#sol-19-2 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-19-2-label"}
::: {.offcanvas-header}
[**解答例（問題 19-2）**]{#sol-19-2-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
...
::: {#sol-19-3 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-19-3-label"}
::: {.offcanvas-header}
[**解答例（問題 19-3）**]{#sol-19-3-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
...
::: {#sol-19-4 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-19-4-label"}
::: {.offcanvas-header}
[**解答例（問題 19-4）**]{#sol-19-4-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
...
::: {#sol-19-5 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-19-5-label"}
::: {.offcanvas-header}
[**解答例（問題 19-5）**]{#sol-19-5-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
