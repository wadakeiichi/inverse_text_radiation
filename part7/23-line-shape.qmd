...
## 導出ノート：古典振動子から吸収断面積と振動子強度 $f$ へ（オンライン版）

第VII部で繰り返し使う線吸収係数 @eq-line-absorption-preview の前因子 $\pi e^2/m_e c$ と振動子強度 $f$ が、どこから来るのかを古典模型から積み上げておく。これは「式を天下りに与えない」ための一筋であり、自然幅の Lorentzian（@eq-lorentzian）と同じ起源を吸収の側から見たものである。

**原子をばねに乗った電子（電気双極子）とみなす。**　入射電磁波の振動電場 $E=E_0 e^{i\omega t}$ に対し、固有角振動数 $\omega_0$・減衰率 $\gamma$ の電子の運動方程式は

$$
\ddot z + \gamma\,\dot z + \omega_0^2\, z = -\frac{e}{m}E_0 e^{i\omega t}.
$$

定常解の振幅から、電子が電磁波から吸収して摩擦（＝再放射）で散逸する仕事率を計算し、それを入射エネルギー流束 $F=(c/8\pi)E_0^2$ で割ると、**古典的な吸収断面積**

$$
\sigma(\nu) = \frac{e^2}{m_e c}\,\frac{\gamma/4\pi}{(\nu-\nu_0)^2+(\gamma/4\pi)^2}
$$ {#eq-classical-cross-section}

が得られる（$\nu\simeq\nu_0$ の共鳴近似）。これは @eq-lorentzian と同じ Lorentzian 形である。振動数で積分すると、形によらず

$$
\int_0^\infty \sigma(\nu)\,d\nu = \frac{\pi e^2}{m_e c} = 2.654\times10^{-2}\ \text{cm}^2\,\text{s}^{-1}
$$ {#eq-integrated-cross-section}

となり、$\gamma$ にも $\nu_0$ にもよらない**普遍定数**になる。

**古典の矛盾と量子補正 $f$。**　@eq-integrated-cross-section が普遍ということは、古典論ではすべての線の積分吸収が同じ＝**どの線も同じ強さ**になってしまう。これは観測（線ごとに強さが違う）と矛盾する。量子力学で双極子の吸収を計算すると、この古典断面積に遷移ごとに異なる無次元係数 $f$ を掛ければよいことがわかる：

$$
\int_0^\infty \sigma(\nu)\,d\nu = \frac{\pi e^2}{m_e c}\,f.
$$ {#eq-fvalue-integral}

この $f$ が**振動子強度**（**oscillator strength**、$f$ 値）で、線の強さを決める。線吸収係数は単位体積あたり $\alpha_\nu^\text{line}=n_l\,\sigma(\nu)=\dfrac{\pi e^2}{m_e c}f\,n_l\,\phi(\nu-\nu_0)$ となり、第5章 @eq-line-absorption-preview の形に一致する ― 前因子 $\pi e^2/m_e c$ の正体がこの古典断面積、$f$ がその量子補正である（上準位占有を無視した真吸収。$n_u\approx0$ は可視・UV 吸収線で標準的な近似で、一般形は第21章 @eq-line-absorption-einstein）。

**等価幅へのつながり。**　弱い線（光学的に薄い）では、柱密度 $N_l=\int n_l\,d\ell$ の原子による等価幅（§21.1）は

$$
W_\lambda \simeq \frac{\pi e^2}{m_e c}\,\frac{\lambda^2}{c}\,N_l\,f
$$ {#eq-weak-line-W}

と $N_l f$ に比例する（成長曲線 §21.6 の**線形部分**）。$f$ が既知なら $W_\lambda$ から柱密度 $N_l$ が逆引きできる ― これが吸収線分光の最も基本的な定量の出発点である。
:::
...
**短答**：$\Delta v_D \simeq \sqrt{2k_B T/m_p} \simeq 12.8$ km/s（**最確速度**＝Doppler パラメータ $b$。標準偏差は $b/\sqrt2\simeq9.1$ km/s）、FWHM = $2\sqrt{\ln 2} \cdot \Delta v_D \simeq 21$ km/s。
...
1. 視線速度分布の最確速度（Doppler パラメータ $b$）に相当する $\Delta v_D=\sqrt{2k_BT/m_p}$ を km/s で求めよ（$m_p=1.67\times10^{-27}$ kg。標準偏差は $b/\sqrt2$）。
...
[解答例を見る（問題 23-1）](#sol-23-1){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::
...
::: {.content-visible when-format="html"}
[解答例を見る（問題 23-2）](#sol-23-2){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::
...
::: {.content-visible when-format="html"}
[解答例を見る（問題 23-3）](#sol-23-3){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::
...
::: {.content-visible when-format="html"}
[解答例を見る（問題 23-4）](#sol-23-4){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

## 解答例 {#sec-line-shape-solutions .unnumbered}

各問の「解答例を見る」ボタンを押すと、右側のパネルに解答例が表示され、問題文と見比べられる（背景は暗くならず本文はそのまま読める）。印刷版・EPUB版では下に順に掲載される。

::: {.content-visible when-format="html"}
[問題 23-1](#sol-23-1){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 23-2](#sol-23-2){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 23-3](#sol-23-3){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 23-4](#sol-23-4){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

::: {#sol-23-1 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-23-1-label"}
::: {.offcanvas-header}
[**解答例（問題 23-1）**]{#sol-23-1-label .offcanvas-title}
...
::: {#sol-23-2 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-23-2-label"}
::: {.offcanvas-header}
[**解答例（問題 23-2）**]{#sol-23-2-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
...
::: {#sol-23-3 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-23-3-label"}
::: {.offcanvas-header}
[**解答例（問題 23-3）**]{#sol-23-3-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
...
::: {#sol-23-4 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-23-4-label"}
::: {.offcanvas-header}
[**解答例（問題 23-4）**]{#sol-23-4-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
