...
![同じ原子遷移が観測の幾何条件で吸収線にも輝線にもなる三つのケース（第19章 §19.3 で導入した図を、§20.4 で源泉関数の文脈で再掲）。温度勾配と背景光の組合せがどう線形態を決めるかが視覚的に分かる。](../images/line-formation-geometry.svg){#fig-line-geometry-lte width=88%}
...
### 問題 20-1　線源泉関数が Bν(Tex) になる {#ex-20-1 .unnumbered}

[★ 難易度：☆☆ ] [tag:導出補完]

§20.3 は線源泉関数を励起温度 $T_{\rm ex}$ のプランク関数に書き換えた。導出を埋める。$S_\nu^{\rm line}=\dfrac{A_{ul}n_u}{B_{lu}n_l-B_{ul}n_u}$、Einstein 関係 $A_{ul}/B_{ul}=2h\nu_0^3/c^2$、$g_lB_{lu}=g_uB_{ul}$。

1. Einstein 関係を使って $S_\nu^{\rm line}=\dfrac{2h\nu_0^3/c^2}{(g_un_l)/(g_ln_u)-1}$ を導け。
2. 励起温度を $\dfrac{g_un_l}{g_ln_u}=e^{h\nu_0/k_BT_{\rm ex}}$ で定義し、$S_\nu^{\rm line}=B_{\nu_0}(T_{\rm ex})$ を示せ。
3. $T_{\rm ex}$ が物理的なキネティック温度と一致するのはどんな条件か（LTE）。一致しない場合（non-LTE）に $S_\nu$ がプランク関数でなくなるわけではない理由を述べよ。

**関連**：[§20.3 線源泉関数](#sec-line-source-function)／Einstein 関係の導出は[演習 11-1](../part4/11-einstein-method.qmd#ex-11-1)、ソース関数の基礎は[演習 5-5](../part2/05-radiative-transfer.qmd#ex-5-5)、LTE は[§20.4](#sec-LTE-line-formation)。
...
$$S_\nu^{\rm line}=\frac{A_{ul}n_u}{B_{lu}n_l-B_{ul}n_u}=\frac{A_{ul}/B_{ul}}{\dfrac{B_{lu}n_l}{B_{ul}n_u}-1}=\frac{2h\nu_0^3/c^2}{\dfrac{g_un_l}{g_ln_u}-1}.$$

**(2)** $\dfrac{g_un_l}{g_ln_u}=e^{h\nu_0/k_BT_{\rm ex}}$ を代入すると分母が $e^{h\nu_0/k_BT_{\rm ex}}-1$ となり

$$S_\nu^{\rm line}=\frac{2h\nu_0^3/c^2}{e^{h\nu_0/k_BT_{\rm ex}}-1}=B_{\nu_0}(T_{\rm ex}).$$

**(3)** 占有数が局所キネティック温度 $T_{\rm kin}$ の Boltzmann 分布で書ける（LTE）とき $T_{\rm ex}=T_{\rm kin}$。non-LTE では占有数が Boltzmann から外れて $T_{\rm ex}\ne T_{\rm kin}$ になるが、$T_{\rm ex}$ を「占有比を Boltzmann 形で書いたときの温度」と**定義**する限り、$S_\nu=B_\nu(T_{\rm ex})$ は形式的に常に成り立つ。ただし $T_{\rm ex}$ は物理温度を反映しない。

**答え**：$S_\nu^{\rm line}=\dfrac{2h\nu_0^3/c^2}{(g_un_l)/(g_ln_u)-1}=B_{\nu_0}(T_{\rm ex})$。LTE で $T_{\rm ex}=T_{\rm kin}$、non-LTE でも定義上 $B_\nu(T_{\rm ex})$ の形は保たれる。
:::

### 問題 20-2　誘導放出補正と準位反転（メーザー） {#ex-20-2 .unnumbered}
...
**関連**：[§20.2 線の三つの素過程](#sec-line-three-processes)／誘導放出は[§11.2](../part4/11-einstein-method.qmd#sec-einstein-method-three-processes)、自発/誘導放出の比は[演習 11-2](../part4/11-einstein-method.qmd#ex-11-2)。
...
### 問題 20-3　等価幅と curve of growth の三領域 {#ex-20-3 .unnumbered}

[★ 難易度：☆☆ ] [tag:理解を深める]

§20.1・20.6 の等価幅 $W_\lambda=\int\frac{I_c-I_\lambda}{I_c}d\lambda$ と curve of growth を読む。

1. 等価幅が「線の輪郭の詳細によらない積分量」である利点を述べよ。浅く広い線と深く狭い線が同じ $W$ になりうる理由は。
2. curve of growth の三領域（線形 $W\propto N$、飽和 $W\propto\sqrt{\ln N}$、減衰翼 $W\propto\sqrt{N}$）が、それぞれ光学的深さ $\tau$ のどんな状態に対応するか述べよ。
3. 飽和領域の線から柱密度 $N$ を精度よく決めるのが難しい理由を述べよ。組成解析で複数の強さの線を使う理由と結びつけよ。

**関連**：[§20.1 等価幅](#sec-line-equivalent-width)、[§20.6 curve of growth](#sec-curve-of-growth)／組成解析は[§23.2](../part7/23-line-applications.qmd#sec-line-applications-abundance)、減衰翼は[§22.1](../part7/22-line-shape.qmd#sec-line-shape-natural)。
...
**(3)** 飽和領域では $W$ が $N$ にほとんど鈍感（$\sqrt{\ln N}$）なので、$W$ の小さな測定誤差が $N$ の大きな誤差になる。だから組成解析では弱い線（線形領域）・中間・強い線（減衰翼）を組み合わせ、curve of growth 全体を観測点で押さえて $N$ を精度よく決める。

**答え**：$W$ は輪郭によらぬ積分量。線形($\tau<1$)→飽和($\tau\gg1$)→減衰翼。飽和域は $N$ に鈍感ゆえ複数の強さの線が必要。
:::

### 問題 20-4　LTE で吸収線になる条件 ― 温度勾配と背景光 {#ex-20-4 .unnumbered}

[★ 難易度：☆☆ ] [tag:理解を深める]

§20.4 は線が吸収か輝線かが温度構造と背景光で決まると述べた。LTE（$S_\nu=B_\nu(T)$）で考える。
...
**関連**：[§20.4 LTE 線形成](#sec-LTE-line-formation)／吸収線か輝線かは[演習 5-3](../part2/05-radiative-transfer.qmd#ex-5-3)、Eddington-Barbier は[演習 15-1](../part6/15-non-blackbody.qmd#ex-15-1)。
