...
## 24.1 視線速度 ― 線中心のシフト {#sec-line-applications-velocity}
...
## 24.2 化学組成 ― 等価幅と成長曲線（curve of growth） {#sec-line-applications-abundance}
...
2. **成長曲線（curve of growth）**：第21章 §21.6 から $W \to N$（柱密度）の換算
3. **正規化**：水素柱密度を基準とし、**相対組成 $N_X/N_H$** を求める
4. **太陽組成との比**：[X/H] $= \log_{10}(N_X/N_H) - \log_{10}(N_X/N_H)_\odot$

![成長曲線（curve of growth）― 等価幅 $W_\lambda$ と柱密度 $N$（$\times f\lambda$）の関係。三つの領域がある：**線形部**（弱い線、$W\propto N$。光学的に薄くコアが線形に成長）、**平坦部**（飽和。Doppler コアが飽和し $W\propto\sqrt{\ln N}$ でほとんど増えない）、**ダンピング部**（強い線、$W\propto\sqrt{N}$。Lorentz 減衰翼が成長）。同じ等価幅でも線がどの領域にあるかで密度感度が変わるため、組成解析では弱・中・強の複数の線で曲線全体を押さえる。](../images/curve-of-growth.svg){#fig-curve-of-growth width=88%}
...
## 24.3 温度・密度診断 ― 線比の原理 {#sec-line-applications-diagnostics}
...
::: {.callout-note}
**用語**：これらは **禁制線**（第20章 §20.3、第22章 §22.5）。禁制線が **密度に敏感** なのは、$A_{ul}$ が小さいために臨界密度 $n_\text{crit}=A_{ul}/q_{ul}$（§21.5、演習 20-2）が星雲の密度域まで下がり、$n_e\sim n_\text{crit}$ の **遷移域** に入るため ― この域でだけ、衝突脱励起と自発放出の競合を通じて線強度比が密度に応答する。低密度極限（$n_e\ll n_\text{crit}$）では衝突励起のたびに光子が出て禁制線は効率よく光り、高密度側（$n_e\gg n_\text{crit}$）では衝突脱励起が勝って弱まる。**「禁制線の存在自体が低密度の証拠」** とも言える
...
## 24.4 電離度と化学種の分布 ― イオン化バランス {#sec-line-applications-ionization}

[本文目安：B3]

同じ元素の異なる電離段階（例：H I vs H II、C IV vs C II）の **存在比** から、電離度が読める（電離平衡を定める Saha 式と、それが連続放射に効く仕組みは[第19章 §19.5](../part6/19-continuum-microprocesses.qmd#sec-continuum-saha)）：
...
## 24.5 宇宙論的応用 ― QSO 吸収系と 21 cm 線 {#sec-line-applications-cosmology}
...
## 24.6 系外惑星大気 ― 透過分光と高分散分光 {#sec-line-applications-exoplanet}
...
## 24.7 まとめ ― 線スペクトル逆引き地図の完成 {#sec-line-applications-summary}
...
|---|---|---|---|
| $\nu_0$ | 線振動数 | 原子・分子のエネルギー準位 | 22 |
| $\nu_0$ シフト | 中心シフト | 視線速度、宇宙論赤方偏移 | 24 |
| $f_{lu}$ | 線強度（と $n_l$ から） | 遷移行列要素（量子力学） | 22 |
| $n_l$ | 等価幅 | 柱密度、化学組成 | 21, 24 |
| $n_l$ ratio | 線比 | 励起温度、電子温度・密度、電離度 | 21, 24 |
| $\phi(\nu-\nu_0)$ 幅 | Doppler 幅 | キネティック温度、乱流速度 | 23 |
| $\phi(\nu-\nu_0)$ 翼 | 翼の形 | 密度、圧力 | 23 |
| $\phi(\nu-\nu_0)$ 分裂 | Zeeman 分裂 | 磁場 | 23 |
...
### 問題 24-1　視線速度と系外惑星の検出 {#ex-24-1 .unnumbered}
...
**関連**：[§24.1 視線速度](#sec-line-applications-velocity)／Doppler シフトは[§23.2](../part7/23-line-shape.qmd#sec-line-shape-doppler)、宇宙膨張と赤方偏移は[演習 16-2](../part6/16-cmb.qmd#ex-16-2)。

::: {.content-visible when-format="html"}
[解答例を見る（問題 24-1）](#sol-24-1){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

### 問題 24-2　線比による温度・密度診断 {#ex-24-2 .unnumbered}

[★ 難易度：☆☆ ] [tag:理解を深める]

§24.3 の禁制線比による診断。[O III] $\lambda$4363/$\lambda$5007（温度）と [S II] $\lambda$6716/$\lambda$6731（密度）。
...
**関連**：[§24.3 温度・密度診断](#sec-line-applications-diagnostics)／禁制線と臨界密度は[演習 20-2](../part7/20-where-lines.qmd#ex-20-2)、Boltzmann 占有は[演習 7-4](../part3/07-statistical-mechanics.qmd#ex-7-4)。

::: {.content-visible when-format="html"}
[解答例を見る（問題 24-2）](#sol-24-2){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

### 問題 24-3　高赤方偏移宇宙 ― Lyα 森と 21 cm {#ex-24-3 .unnumbered}
...
**関連**：[§24.5 宇宙論的応用](#sec-line-applications-cosmology)／CMB は[第16章](../part6/16-cmb.qmd)（[第VI部 演習](../part6/16-cmb.qmd)）、21 cm の起源は[§20.4](../part7/20-where-lines.qmd#sec-where-lines-21cm)（[演習 20-1](../part7/20-where-lines.qmd#ex-20-1)）。

::: {.content-visible when-format="html"}
[解答例を見る（問題 24-3）](#sol-24-3){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

## 解答例 {#sec-line-applications-solutions .unnumbered}

各問の「解答例を見る」ボタンを押すと、右側のパネルに解答例が表示され、問題文と見比べられる（背景は暗くならず本文はそのまま読める）。印刷版・EPUB版では下に順に掲載される。

::: {.content-visible when-format="html"}
[問題 24-1](#sol-24-1){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 24-2](#sol-24-2){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 24-3](#sol-24-3){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

::: {#sol-24-1 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-24-1-label"}
::: {.offcanvas-header}
[**解答例（問題 24-1）**]{#sol-24-1-label .offcanvas-title}
...
::: {#sol-24-2 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-24-2-label"}
::: {.offcanvas-header}
[**解答例（問題 24-2）**]{#sol-24-2-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
...
::: {#sol-24-3 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-24-3-label"}
::: {.offcanvas-header}
[**解答例（問題 24-3）**]{#sol-24-3-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
