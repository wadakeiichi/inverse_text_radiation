...
::: {.callout-note}
**統一公式から見た吸収線**：吸収線が暗く見えるのは、第5章 §5.11 の統一公式 @eq-unified-spectrum で $k_\lambda$ が線中心 $\nu_0$ で大きくなる（$k_\lambda>k_R$）ためである。このとき見える温度は $T=T_\mathrm{e}[\tfrac12(1+k_R/k_\lambda)]^{1/4}<T_\mathrm{e}$ となり、線中心では**浅く低温の層**しか見通せないので連続光より暗くなる。連続吸収（滑らかな $k_\lambda$）と線吸収（鋭い $k_\lambda(\nu_0)$）は、同じ式の $k_\lambda$ の構造の違いにすぎない ― これが第15章（連続のずれ）と本章（線）を貫く視点である。線中心で $k_\lambda\to\infty$ でも、見えるのは最表面 $T_0\simeq0.84\,T_\mathrm{e}$ までで線の底は飽和する（§5.11 の問い）。
:::
...
つまり「衝突率 vs 放射率」の大小が LTE/non-LTE を分ける ― 統計平衡の抽象論の中身は、この一本の不等式に凝縮できる。許容線（大きい $A_{ul}$）は $n_\text{crit}$ が高く non-LTE になりやすい一方、**禁制線**（小さい $A_{ul}$、第22章 §22.5）は $n_\text{crit}$ が低く、より低い密度で熱平衡化に近づく。ただし典型的な電離星雲（$n_e\sim10^2$–$10^4$ cm$^{-3}$）では、多くの禁制線もなお $n_e\lesssim n_\text{crit}$（例：[O III] $\lambda$5007 で $n_\text{crit}\simeq7\times10^5$ cm$^{-3}$）の non-LTE 側にある。むしろ $n_e\sim n_\text{crit}$ の **遷移域にあること** こそが使い道で、この域でだけ線強度が密度に敏感になる ― これを逆手に取って電子密度を測るのが、第24章 §24.3 の [S II] $\lambda$6716/$\lambda$6731 **線比診断**（多準位の具体例）である。
:::
...
§21.1・21.6 の等価幅 $W_\lambda=\int\frac{I_c-I_\lambda}{I_c}d\lambda$ と成長曲線（curve of growth）を読む。
