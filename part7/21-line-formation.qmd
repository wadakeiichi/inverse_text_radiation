...
::: {.chapter-overview}
**この章の主題**：第20章で見たさまざまな線スペクトルは、すべて「**原子・分子の離散的遷移と、その場の放射場との相互作用**」の結果として理解できる。本章では、第11章で導入した **Einstein 係数** $A_{ul}, B_{lu}, B_{ul}$ を線スペクトルの主役として再登場させ、**線吸収・線輝線・線源泉関数** を逆引きで体系化する。これは連続スペクトルの第4-5章（放射輸送と源泉関数）の **線スペクトル版** にあたる。
:::
...
**注意（観測）**：等価幅は **観測スペクトル分解能の限界以上の線** には鋭敏。一方、強く飽和した線（線芯が完全に黒）では、追加の物質を加えても等価幅はゆっくりとしか増えない（§21.6 の curve of growth の「飽和域」）。等価幅から物理量を読むには、線がどの「成長領域」にあるかを意識する必要がある
:::
...
## 21.3 線源泉関数 ― 連続スペクトルから線スペクトルへ {#sec-line-source-function}
...
::: {.callout-note}
**用語**：**励起温度** $T_\text{ex}$ は、観測者が観測される線の強度から **逆引き** で求める「上下準位の占有比を Boltzmann 分布で書いたときの温度」。物理的に物質と熱平衡している温度とは限らない（LTE / non-LTE の区別、§21.4-§21.5）

**対応（観測）**：観測される線の輝度温度から $T_\text{ex}$ を測れる ― これが線スペクトルから物理状態を逆引きする最初のステップ
:::

## 21.4 LTE 線形成 ― 線源泉関数 ≈ $B_\nu(T_\text{kin})$ {#sec-LTE-line-formation}
...
![同じ原子遷移が観測の幾何条件で吸収線にも輝線にもなる三つのケース（第20章 §20.3 で導入した図を、§21.4 で源泉関数の文脈で再掲）。温度勾配と背景光の組合せがどう線形態を決めるかが視覚的に分かる。](../images/line-formation-geometry.svg){#fig-line-geometry-lte width=88%}
...
## 21.6 curve of growth ― 等価幅から柱密度を測る {#sec-curve-of-growth}
...
::: {.callout-note}
**対応（観測）**：恒星・QSO 吸収線スペクトルの解析で、curve of growth は **観測等価幅 → 柱密度** を求める基本道具。化学組成、QSO 視線方向の星間吸収系の柱密度測定などに使われる。第24章 §24.2 で本格的に応用する
:::
...
**関連**：[§21.3 線源泉関数](#sec-line-source-function)／Einstein 関係の導出は[演習 11-1](../part4/11-einstein-method.qmd#ex-11-1)、ソース関数の基礎は[演習 5-5](../part2/05-radiative-transfer.qmd#ex-5-5)、LTE は[§21.4](#sec-LTE-line-formation)。
...
### 問題 21-3　等価幅と curve of growth の三領域 {#ex-21-3 .unnumbered}

[★ 難易度：☆☆ ] [tag:理解を深める]

§21.1・20.6 の等価幅 $W_\lambda=\int\frac{I_c-I_\lambda}{I_c}d\lambda$ と curve of growth を読む。
...
**関連**：[§21.1 等価幅](#sec-line-equivalent-width)、[§21.6 curve of growth](#sec-curve-of-growth)／組成解析は[§24.2](../part7/24-line-applications.qmd#sec-line-applications-abundance)、減衰翼は[§23.1](../part7/23-line-shape.qmd#sec-line-shape-natural)。
