...
## 20.1 太陽の Fraunhofer 線 ― 線スペクトル発見の起点 {#sec-where-lines-fraunhofer}
...
## 20.2 恒星の吸収線とスペクトル分類 {#sec-where-lines-stellar-classification}
...
## 20.3 星雲の輝線スペクトル ― 吸収線とは何が違うのか {#sec-where-lines-emission}

[本文目安：B2-B3]

![同じ原子遷移が、観測の幾何条件によって **吸収線** にも **輝線** にも見える仕組み。(a) 背後に明るい連続光源があってその前を冷たいガスが覆う → 吸収線（恒星大気）、(b) 背景に連続光がなく、励起されたガス自体を観測 → 輝線（星雲）、(c) 温度勾配のある大気 → 表面付近では冷たく見えるため吸収線。Kirchhoff の三法則の現代的視覚化、§21.4 でも再登場する。](../images/line-formation-geometry.svg){#fig-line-geometry width=92%}
...
::: {.callout-note}
**用語**：**禁制線**（**forbidden line**）― [O III] $\lambda$5007 のように、四角括弧で囲んだ線。原子物理の選択則（第22章 §22.5）では本来「禁じられた」遷移だが、地上では衝突で先に脱励起されるため見えない一方、**極低密度** の星雲環境では時間が許されて自発放出が起こる。HII 領域の代表線である [O III] が「禁制線」を冠するのは、**星雲が地上では実現できない希薄環境であることの直接観測的証拠**

**観測との対応**：禁制線は **通常の実験室条件（高密度）では衝突脱励起で潰されて見えにくい遷移**。ただし極低密度の系では地球上でも観測される ― 例えば **オーロラの $[\text{O I}]$ $\lambda$5577**（緑光）は地球超高層大気の典型的な禁制線で、EBIT 等の希薄プラズマ実験室でも測られる。むしろ「禁制線が出る $\Leftrightarrow$ 低密度」という関係を読みとる手段として、星雲・上層大気・宇宙環境の共通の道具になる。第24章 §24.3 で電子密度・温度の診断に活躍する
:::

## 20.4 21 cm 線 ― 中性水素の宇宙論的トレーサー {#sec-where-lines-21cm}
...
**注意（前因子の扱い）**：中心地図の前因子 $\pi e^2/m_e c$ は、古典 Lorentz 振動子に対応する **電気双極子（E1）遷移** の積分吸収断面積である（§22.4b）。21 cm 線は基底状態超微細構造の **磁気双極子（M1）遷移** なので、この前因子と通常の振動子強度 $f_{lu}$ をそのまま当てはめることはできない。21 cm 線の吸収・放出強度は、測定された自発放出率 $A_{ul} \simeq 2.9 \times 10^{-15}$ s$^{-1}$ から Einstein 関係を介して読み替える（@eq-A-f-conversion の関係を E1 に限らず一般化し、$A_{ul}$ ベースで $B_{lu}$ を求める）必要がある。
:::

## 20.5 分子線 ― 電波・サブミリの主役 {#sec-where-lines-molecular}
...
## 20.6 AGN・クェーサーの線 ― 広線と狭線の物語 {#sec-where-lines-agn}
...
## 20.7 線スペクトルの逆引きへ向けて {#sec-where-lines-toward-inverse}
...
### 問題 20-1　21 cm 線 ― 極端に弱い遷移がなぜ観測できるか {#ex-20-1 .unnumbered}
...
**関連**：[§20.4 21 cm 線](#sec-where-lines-21cm)／中心地図の $n_l$ は[§21.3](../part7/21-line-formation.qmd#sec-line-source-function)、超微細構造の M1 遷移は[§26.5](../part8/26-qed-minimum.qmd#sec-qed-minimum-selection)、宇宙論応用は[§24.5](../part7/24-line-applications.qmd#sec-line-applications-cosmology)。

::: {.content-visible when-format="html"}
[解答例を見る（問題 20-1）](#sol-20-1){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

### 問題 20-2　禁制線は「低密度の証拠」 {#ex-20-2 .unnumbered}

[★ 難易度：☆☆ ] [tag:理解を深める]

§20.3 は [O III] $\lambda$5007 等の禁制線が星雲で強く見える一方、地上実験室では見えにくいと述べた。
...
**関連**：[§20.3 星雲の輝線](#sec-where-lines-emission)／選択則と禁制線の次数は[§22.5](../part7/22-atomic-physics.qmd#sec-atomic-selection-rules)（[演習 22-3](../part7/22-atomic-physics.qmd#ex-22-3)）、密度診断は[§24.3](../part7/24-line-applications.qmd#sec-line-applications-diagnostics)（[演習 24-2](../part7/24-line-applications.qmd#ex-24-2)）。

::: {.content-visible when-format="html"}
[解答例を見る（問題 20-2）](#sol-20-2){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

### 問題 20-3　AGN 広線からブラックホール質量 {#ex-20-3 .unnumbered}
...
**関連**：[§20.6 AGN・クェーサーの線](#sec-where-lines-agn)／ブラックホール近傍の相対論的効果は[§18.5](../part6/18-accretion-high-energy.qmd#sec-black-hole-effects)（[第VI部 演習](../part6/18-accretion-high-energy.qmd)）、線幅は[§23.5](../part7/23-line-shape.qmd#sec-line-shape-macro)。

::: {.content-visible when-format="html"}
[解答例を見る（問題 20-3）](#sol-20-3){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

## 解答例 {#sec-where-lines-solutions .unnumbered}

各問の「解答例を見る」ボタンを押すと、右側のパネルに解答例が表示され、問題文と見比べられる（背景は暗くならず本文はそのまま読める）。印刷版・EPUB版では下に順に掲載される。

::: {.content-visible when-format="html"}
[問題 20-1](#sol-20-1){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 20-2](#sol-20-2){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 20-3](#sol-20-3){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

::: {#sol-20-1 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-20-1-label"}
::: {.offcanvas-header}
[**解答例（問題 20-1）**]{#sol-20-1-label .offcanvas-title}
...
::: {#sol-20-2 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-20-2-label"}
::: {.offcanvas-header}
[**解答例（問題 20-2）**]{#sol-20-2-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
...
::: {#sol-20-3 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-20-3-label"}
::: {.offcanvas-header}
[**解答例（問題 20-3）**]{#sol-20-3-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
