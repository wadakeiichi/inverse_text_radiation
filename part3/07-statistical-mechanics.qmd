...
つまり、**分配関数を計算できれば熱力学はほぼ完結する**。逆に言えば、実際の計算の核心はミクロ状態の和 @eq-partition-function を実行する点にある。
...
::: {.callout-note}
**零点エネルギー $\hbar\omega/2$ の扱いについて**：本章では $\epsilon_n = (n+1/2)\hbar\omega$ と書いて零点項を残したが、Planck の原論文（1900）は $\epsilon_n = n h\nu$ と書いた（第10章でこの形を再現する）。零点項の有無は **観測量である占有因子 $1/(e^{\hbar\omega/k_BT}-1)$ には影響しない**（零点エネルギーは温度によらない定数で、温度微分・分配関数の対数微分で消える）。両者の物理的意味（古典的振動子の量子化 vs エネルギー量子の数え上げ）と、零点項が観測可能になる場面（真空揺らぎ、Casimir 効果、自発放出、§25.3）は第IV部と第VIII部で扱う
:::
...
::: {.callout-note}
**観測との対応**：観測される線スペクトル強度比から温度を読むのは、まさにこの Boltzmann 分布が成り立っているという仮定（LTE）に基づいている。LTE が破れる場面（non-LTE、第5章 §5.5）では @eq-boltzmann-hydrogen は使えない。
:::

::: {.callout-note}
**使えるようになった道具**（§7.3）：Boltzmann 分布 @eq-boltzmann-distribution は、観測される複数の線の強度比から温度を読み出す「**励起温度**」推定の基礎式である。第VII部・第24章で本格的に応用する。
:::
...
1. 系がエネルギー $E_i$ の状態にある確率は、熱浴が残りのエネルギー $E_\mathrm{tot}-E_i$ を取る多重度に比例する：$P_i\propto\Omega_\mathrm{bath}(E_\mathrm{tot}-E_i)$。これを $S_\mathrm{bath}=k_B\ln\Omega_\mathrm{bath}$ で書き換えよ。
2. $E_i\ll E_\mathrm{tot}$ として $S_\mathrm{bath}(E_\mathrm{tot}-E_i)$ を $E_i$ について1次まで展開し、$1/T=\partial S_\mathrm{bath}/\partial E$ を使って $P_i\propto e^{-E_i/k_BT}$ を導け。
...
[解答例を見る（問題 7-1）](#sol-7-1){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::
...
::: {.content-visible when-format="html"}
[解答例を見る（問題 7-2）](#sol-7-2){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

### 問題 7-3　エネルギー揺らぎ (ΔE)²=kBT²Cv {#ex-7-3 .unnumbered}
...
::: {.content-visible when-format="html"}
[解答例を見る（問題 7-3）](#sol-7-3){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::
...
**関連**：[§7.3 Boltzmann 分布](#sec-statistical-mechanics-boltzmann)／恒星の色と温度は[§1.3](../part1/01-where-blackbody.qmd#sec-where-blackbody-stars)、線強度からの温度推定は[第24章](../part7/24-line-applications.qmd)、原子準位は[第22章](../part7/22-atomic-physics.qmd)。

::: {.content-visible when-format="html"}
[解答例を見る（問題 7-4）](#sol-7-4){.btn .btn-outline-secondary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

## 解答例 {#sec-statistical-mechanics-solutions .unnumbered}

各問の「解答例を見る」ボタンを押すと、右側のパネルに解答例が表示され、問題文と見比べられる（背景は暗くならず本文はそのまま読める）。印刷版・EPUB版では下に順に掲載される。

::: {.content-visible when-format="html"}
[問題 7-1](#sol-7-1){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 7-2](#sol-7-2){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 7-3](#sol-7-3){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"} [問題 7-4](#sol-7-4){.btn .btn-outline-primary .btn-sm data-bs-toggle="offcanvas" role="button"}
:::

::: {#sol-7-1 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-7-1-label"}
::: {.offcanvas-header}
[**解答例（問題 7-1）**]{#sol-7-1-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
**(1)** 系が状態 $i$（エネルギー $E_i$）にあるとき、全体のミクロ状態数は熱浴の多重度 $\Omega_\mathrm{bath}(E_\mathrm{tot}-E_i)$ に等しい。等確率の原理から $P_i\propto\Omega_\mathrm{bath}(E_\mathrm{tot}-E_i)=e^{S_\mathrm{bath}(E_\mathrm{tot}-E_i)/k_B}$。

**(2)** $E_i\ll E_\mathrm{tot}$ で Taylor 展開：
...
よって $P_i\propto e^{S_\mathrm{bath}(E_\mathrm{tot})/k_B}e^{-E_i/k_BT}\propto e^{-E_i/k_BT}$。

**(3)** $\sum_i P_i=1$ より $P_i=\dfrac{e^{-E_i/k_BT}}{\sum_j e^{-E_j/k_BT}}=\dfrac{e^{-E_i/k_BT}}{Z}$、$Z=\sum_j e^{-E_j/k_BT}$。

**答え**：$P_i\propto\Omega_\mathrm{bath}(E_\mathrm{tot}-E_i)$ を1次展開し $1/T=\partial S/\partial E$ を使うと $P_i=e^{-E_i/k_BT}/Z$。
:::
:::

::: {#sol-7-2 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-7-2-label"}
::: {.offcanvas-header}
[**解答例（問題 7-2）**]{#sol-7-2-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
...
::: {#sol-7-3 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-7-3-label"}
::: {.offcanvas-header}
[**解答例（問題 7-3）**]{#sol-7-3-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
...
::: {#sol-7-4 .offcanvas .offcanvas-end tabindex="-1" data-bs-backdrop="false" data-bs-scroll="true" aria-labelledby="sol-7-4-label"}
::: {.offcanvas-header}
[**解答例（問題 7-4）**]{#sol-7-4-label .offcanvas-title}
[ ]{.btn-close data-bs-dismiss="offcanvas" aria-label="閉じる"}
:::
::: {.offcanvas-body}
